...
<section id="backoffice-container">
  ...
  <p class="texte miette-de-pain tres-petit"
    id="breadcrumb"><i class="fas fa-home"></i>/Admin/Livres/<?= LANG["admin"]["author_edit"] ?></p>

  <div class="row">
    <div class="col-1 center"></div>
    <div class="col-10 center">
      <h1 class="texte souligne-bleu gros en-majuscule">
        <?= LANG["admin"]["author_edit_alt"] ?> <?= $_POST["author"]["firstname"] ?> <?= $_POST["author"]["lastname"] ?>
      </h1>
      <form method="post"
        action="/admin/authors/edit"
        enctype="multipart/form-data">
        <br><br>
        <input type="hidden"
          name="idAuthor"
          value="<?= $_POST["author"]["id"] ?>"><input type="hidden"
          name="oldPhoto"
          value="<?= $_POST["author"]["profilePicture"] ?>">
        <input type="hidden"
          name="oldBanner"
          value="<?= $_POST["author"]["bannerPicture"] ?>">
        <div class="row">
          ...
          <div class="col-3"><input type="text"
              ...
              value="<?= $_POST["author"]["firstname"] ?>"
              placeholder="<?= LANG["admin"]["author_firstname"] ?>"></div>
          <div class="col-3"><input type="text"
              class="input tres-epais bleu fluide"
              name="authorLastname"
              value="<?= $_POST["author"]["lastname"] ?>"
        ...
        <textarea id="authorEditor"
          name="authorEditor"
          style="height: 500px;">
          <?= str_replace("\\", "", $_POST["author"]["content"]) ?>
				</textarea>
        <br>
        <div class="row">
          <div class="col-5"></div>
